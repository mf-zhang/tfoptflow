...
class ModelPWCNet(ModelBase):
        ...
        with tf.variable_scope(tf.get_variable_scope()) as outer_scope:
            for n, ops_device in enumerate(self.opts['gpu_devices']):
                ...
                with tf.device(assign_to_device(ops_device, self.opts['controller'])), tf.name_scope(f'tower_{n}'):
                    ...
                    loss_unreg = pwcnet_loss(y_tnsr, flow_pyr_tnsr, self.opts, self.batch_weight) # zmf add batch_weight
    ...
    def setup_loss_ops(self):
        ...
        loss_unreg = pwcnet_loss(self.y_tnsr, self.flow_pyr_tnsr, self.opts, self.batch_weight) # zmf add batch_weight
    ...
    def train(self):
        ...
        with self.graph.as_default():
            ...
            while step < self.opts['max_steps'] + 1:
                ...
                feed_dict = {self.x_tnsr: x_adapt, self.y_tnsr: y_adapt, self.batch_weight: batch_weight} # zmf add batch weight
                ...
                if val_batch_size > 0 and step % self.opts['val_step'] == 0:
                    ...
                    for _round in range(rounds):
                        ...
                        batch_weight = 1.0 # zmf add this line
                        # feed_dict = {self.x_tnsr: x_adapt, self.y_tnsr: y_adapt}
                        feed_dict = {self.x_tnsr: x_adapt, self.y_tnsr: y_adapt, self.batch_weight: batch_weight} # zmf add batch weight
